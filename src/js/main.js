...
document.addEventListener('DOMContentLoaded', () => {
  ...
  heroExitAnimation.to(heroSection, {
    opacity: 0.7,
    scale: 0.95,
    ease: 'power1.in',
    force3D: true
  ...
  gsap.set(
    [
      coursesSection.querySelector('.section-title'),
      coursesSection.querySelector('.section-subtitle')
    ],
    {
      opacity: 0,
      y: 30
  ...
  const horizontalScrollTl = gsap.timeline({
    scrollTrigger: {
      ...
      scrub: 1,
      pin: true, // 👈 фіксує wrapper на час анімації
      ...
      onUpdate: (self) => {
        ...
        if (newSlide < totalSlides && currentSlideEl) {
          currentSlideEl.textContent = (newSlide + 1)
            .toString()
            .padStart(2, '0');
        }
    ...
    if (videoEl && playBtn) {
      playBtn.addEventListener('click', () => {
        if (videoEl.paused) {
          ...
          document
            .querySelectorAll('.video-testimonial video')
            .forEach((video) => {
              if (video !== videoEl) {
                video.pause();
                const overlay = video
                  .closest('.video-testimonial')
  ...
  const stackedBlock = document.querySelector('.stacked-block');
  const stackedBlockHeight = stackedBlock.offsetHeight;
  const stackedBlockWidth = stackedBlock.offsetWidth;

  const stackedContainer = document.querySelector('.stacked-container');
  const stackedContainerHeight = stackedContainer.offsetHeight;
  const stackedContainerWidth = stackedContainer.offsetWidth;

  const overlapX =
    (stackedContainerWidth - stackedBlockWidth) / (blocks.length - 1); // should depends on screen sizes
  const overlapY =
  ...
  blocks.forEach((block, i) => {
    t2.to(block, {
      opacity: 1,
      x: overlapX * i, // should depends on screen sizes and blocks count
      y: overlapY * i, // should depends on screen sizes and blocks count
      duration: 15,
      ease: 'power2.out'
    }); // додаємо кожен блок послідовно
    t2.addPause('+=' + 5.5);
  });
});
